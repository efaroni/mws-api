...
function getThrottleBackoffForAction(requestString, quotaRemaining) {
   switch (requestString) {
       case 'ListMatchingProducts':
           return quotaRemaining > 150 ? 50000 : 100000;
       case 'GetCompetitivePricingForASIN':
           return 2000;
       case 'GetLowestOfferListingsForASIN':
           return 2000;
       case 'GetMyFeesEstimate':
           return 2000;
       default:
           return 5000;
   }
...
class AmazonMwsClient {
    ...
    call(req, q, meta) {
        return this.request(req, q, meta).then((response) => {
            ...
            const requestString = req.action;

            let timePerRequest = 0;

            if (headers['x-mws-quota-remaining']) {
                const currentTime = new Date(Date.now());
                const resetTime = new Date(headers['x-mws-quota-resetson']);
                const diff = resetTime - currentTime;
                // subtract five to have some request room
                const quotaRemaining = headers['x-mws-quota-remaining'] - 5;
                // below logic is better throttling than waiting for the aws to
                // throttle for you
                if (quotaRemaining > 0) {
                    timePerRequest = diff / quotaRemaining;
                }
            }
            ...
            return parseString(body, { explicitArray: false, mergeAttrs: true })
                .then((data) => {
                  ...
                  if (!meta.next || !req.options.isArray) {
                      return { result, metadata, timePerRequest };
                  }
                ...
                .catch({ code: 'RequestThrottled' }, (err) => {
                    ...
                    // this backoff works better since exponential backoffs are prone
                    // to cause network resets from AWS
                    const backoffDuration = getThrottleBackoffForAction(requestString, quotaRemaining);
